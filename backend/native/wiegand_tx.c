...
int send_wiegand_pulses(unsigned int d0_pin, unsigned int d1_pin, const char* bits, int pulse_us) {
    ...
    for (int i = 0; i < bit_count; i++) {
        ...
        if (i < bit_count - 1) usleep_precise(INTER_PULSE_MS * 1000);
        if ((i + 1) % 8 == 0 || i == bit_count - 1) printf("  %d/%d bits\n", i + 1, bit_count);
    }
...
int main(int argc, char *argv[]) {
    if (argc < 6) {
        ...
        fprintf(stderr, "Example: sudo %s 23 24 123 45678 26 50\n", argv[0]);
    ...
    if (d0 > 27 || d1 > 27 || d0 == d1) {
        fprintf(stderr, "Error: Invalid GPIO pins\n");
        return 1;
    }
