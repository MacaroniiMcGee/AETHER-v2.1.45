/**
 * server.js – libgpiod-backed backend with automation + Wiegand + OSDP + NFC + bridge + emulations persistence + FORMAT API + GPIO QUEUE
...
gpioQueue.on('board-recovered', () => {
  console.log('[Server] ✅ Board recovered successfully!');
});

gpioQueue.on('board-needs-power-cycle', () => {
  console.error('[Server] ⚠️  CRITICAL: Board needs POWER CYCLE - automatic recovery failed');
...
async function initializeSystems() {
  try {
    wiegandManager = new WiegandManager();
    await wiegandManager.initialize();
    console.log('[Wiegand] ✓ System initialized successfully');
  } catch (err) {
    console.error('[Wiegand] ✗ Failed to initialize:', err && err.message ? err.message : err);
  ...
  try {
    ...
    console.log('[OSDP] ✓ System initialized successfully');
    ...
    console.error('[OSDP] ✗ Failed to initialize:', err && err.message ? err.message : err);
  ...
  try {
    ...
    if (!enableNfc) {
      ...
      if (nfcManager && nfcManager.enabled) {
        console.log('[NFC] ✓ PN532 NFC Reader initialized successfully');
  ...
  if (nfcManager && nfcManager.enabled && osdpManager) {
    try {
      ...
      console.log('[NFC-OSDP Bridge] ✓ Bridge initialized');
  ...
  try {
    ...
    console.error('[Automation] ✗ Failed to initialize:', err && err.message ? err.message : err);
...
app.post('/api/osdp/baudrate', async (req, res) => {
  ...
  try {
    ...
    const result = await osdpManager.changeBaudRate(baudRate);
    res.json({ success: true, ...result });
  } catch (err) {
    res.status(400).json({ success: false, error: err && err.message ? err.message : String(err) });
  }
...
app.get('/api/osdp/formats', (req, res) => {
  if (!osdpManager) {
    const fallback = [
      ...
      { id: 'wiegands56', label: 'Wiegand 56-bit', bitCount: 56 },
